...
pub(super) fn describe_mul_f64(x : f64,y : f64) -> Step {
    const BASE : u32 = 10;

    let mut step = Step::new("lets start (TODO GIVE BETTER STARTING INSTRUCTION)".to_string());

    let (c_aligned,padding,longest_decimal) = align(x,y,"*");
  ...
  //  let mut x_dec_encounted = false;
   // let mut y_dec_encounted = false;
    ...
    let substeps = describe_add_numbers(&addition_numbers,padding);
    step.add_substeps(substeps);
...
pub fn align(x : f64,y : f64,op_str : &str) -> (Vec<String>,usize,usize) {
